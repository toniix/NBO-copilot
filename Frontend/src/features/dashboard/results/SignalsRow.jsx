...
const riskLabel = (pct, label) => {
  if (label === "riesgo_alto")
    return { text: "Riesgo de fuga alto", tone: "text-[#DE3B2E]" };
  if (label === "riesgo_medio_alto")
    return { text: "Riesgo de fuga medio-alto", tone: "text-[#DE3B2E]" };
  if (label === "riesgo_medio_bajo")
    return { text: "Riesgo de fuga medio-bajo", tone: "text-[#B8860B]" };
  if (label === "riesgo_bajo")
    return { text: "Riesgo de fuga bajo", tone: "text-[#2E9E5B]" };

  if (pct <= 25) return { text: "Riesgo de fuga bajo", tone: "text-[#2E9E5B]" };
  if (pct <= 60)
    return { text: "Riesgo de fuga medio", tone: "text-[#B8860B]" };
  return { text: "Riesgo de fuga alto", tone: "text-[#DE3B2E]" };
};
...
const SignalsRow = ({ clientData }) => {
  ...
  const risk = riskLabel(churnPct, clientData.churnLabel);
    ...
    >
      ...
      >
        <div className="flex items-center justify-between gap-4">
          ...
          <p className="text-right text-xs leading-5 text-slate-500">
            {clientData.churnLabel
              ? `Diagnóstico: ${clientData.churnLabel}`
              : "Probabilidad estimada de fuga en los próximos meses."}
          </p>
        </div>
      </SignalCard>
      ...
      <SignalCard
        ...
        footer={
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-semibold text-[#2E9E5B]">
      ...
      >
        <div className="flex items-center justify-between gap-4">
          ...
          <p className="text-right text-xs leading-5 text-slate-500">
            Qué tan probable es que el cliente acepte un plan Movistar Total
            (fibra + móvil).
          </p>
        </div>
      </SignalCard>
      ...
      >
        <div className="space-y-3">
          ...
          <p className="text-xs leading-5 text-slate-500">
            {clientData.scores?.mt_propensity != null &&
            clientData.scores?.churn_risk != null
